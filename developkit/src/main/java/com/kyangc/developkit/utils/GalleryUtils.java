...
public class GalleryUtils {
    ...
    public static String saveBitmapAsPNG(Bitmap bitmap, String filePath, String fileName) throws
            IOException {
        File file = new File(filePath);
        if (!file.exists()) {
            Timber.d("Directory doesn't exist, make directory: " + file.mkdirs());
        }

        file = new File(filePath, fileName + ".png");
        if (file.exists()) {
            Timber.d("File exists, delete it: " + file.delete());
        }
        ...
        try {
            if (bitmap.compress(Bitmap.CompressFormat.PNG, 100, out)) {
                out.flush();
                out.close();
                Timber.d("-->Compress success" + file.getAbsolutePath());
